...
namespace SariaMod.Items.Amethyst
{
    ...
    public class AmethystSariaMinion : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.rand.NextBool(30))//controls the speed of when the sparkles spawn
            {
                ...
                Dust.NewDust(new Vector2(projectile.Center.X + radius * (float)Math.Cos(angle), (projectile.Center.Y - 10) + radius * (float)Math.Sin(angle)), 0, 0, ModContent.DustType<Shadow>(), 0f, 0f, 0, default(Color), 1.5f);
            }
            //////////////////////////////faces start
            ...
            if (base.projectile.localAI[0] == 0f)
            {
                ...
                if (player.ownedProjectileCounts[ModContent.ProjectileType<GhostBarrier>()] <= 0f)
                {
                    {
                        Projectile.NewProjectile(base.projectile.Center + Utils.RandomVector2(Main.rand, -24f, 24f), Vector2.One.RotatedByRandom(6.2831854820251465) * 4f, ModContent.ProjectileType<GhostBarrier>(), base.projectile.damage, base.projectile.knockBack, player.whoAmI, base.projectile.whoAmI);
                    }
                }
                for (int j = 0; j < 1; j++) //set to 2
                {
                    Projectile.NewProjectile(base.projectile.Center + Utils.RandomVector2(Main.rand, -24f, 24f), Vector2.One.RotatedByRandom(6.2831854820251465) * 1f, ModContent.ProjectileType<ZtargetAM>(), base.projectile.damage, base.projectile.knockBack, player.whoAmI, base.projectile.whoAmI);
                }
            ...
            if ((player.HasBuff(ModContent.BuffType<AmethystSariaBuff>()) && (player.ownedProjectileCounts[ModContent.ProjectileType<AMSariaMinion>()] > 0f)))
            {
                projectile.Kill();
            }
            ...
            Lighting.AddLight(projectile.Center, Color.GhostWhite.ToVector3() * 2f);
            ...
            if (projectile.frameCounter >= frameSpeed)
            {
                ...
                if (projectile.frame >= Main.projFrames[ModContent.ProjectileType<RubySariaMinion>()]) //error here! you had the wrong projectile id, so the animation did not use the right frames
                {
                    projectile.frame = 0;
                ...
                if (projectile.ai[0] == 3) //recovery setup
                {
                    ...
                    projectile.ai[1] = 15; //4 cycles recovery between shots, adjust this for how long she waits between swipes
                    projectile.ai[0] = 4;
                }
