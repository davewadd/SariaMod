		...
		public override void AI()
		{
			...
			if (projectile.timeLeft == 10)
			{
				...
				if ((player.ownedProjectileCounts[ModContent.ProjectileType<EmeraldfairySilver>()] <= 0f))
				{
					...
					Projectile.NewProjectile(projectile.Center, Utils.NextVector2Circular(Main.rand, 12f, 12f), ModContent.ProjectileType<ReturnBallSilver>(), projectile.damage, projectile.knockBack, player.whoAmI);
					Projectile.NewProjectile(projectile.Center, Utils.NextVector2Circular(Main.rand, 0, 2), ModContent.ProjectileType<EmeraldfairySilver>(), projectile.damage, projectile.knockBack, player.whoAmI);
				}
				else
                {
