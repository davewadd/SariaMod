...
{
    public abstract class BaseSweetspot : ModProjectile
    {
        protected virtual int ProjectileWidth => 100;
        protected virtual int ProjectileHeight => 150;
        protected virtual int DamageDivisor => 0;
        protected virtual int SpikeFollowType => ModContent.ProjectileType<Emeraldspike>();
        protected virtual int KillConditionSpikeType => SpikeFollowType;
        protected virtual bool HasProjectileKillLoop => false;
        protected virtual int HitCheckSpawnType => 0;
        protected virtual int LaunchHitBoxType => ModContent.ProjectileType<LaunchHitBox>();
        ...
        {
            base.Projectile.width = ProjectileWidth;
            base.Projectile.height = ProjectileHeight;
        ...
        public override void AI()
        {
            ...
            if (HasProjectileKillLoop)
            {
                for (int i = 0; i < 1000; i++)
                {
                    if (Main.projectile[i].active && i != base.Projectile.whoAmI && player.immune == false && Main.projectile[i].Hitbox.Intersects(base.Projectile.Hitbox) && Main.projectile[i].active && ((!Main.projectile[i].friendly && Main.projectile[i].hostile) || (Main.projectile[i].trap)))
                    {
                        ...
                        if (HitCheckSpawnType > 0 && Main.myPlayer == Projectile.owner) Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.position.X + 0, player.position.Y + 0, 0, 0, HitCheckSpawnType, (int)(Projectile.damage), 0f, Projectile.owner, player.whoAmI, Projectile.whoAmI);
                    }
                }
            }
            ...
            if (player.velocity.Y <= 0 || player.ownedProjectileCounts[KillConditionSpikeType] < 1f)
            {
                if (Main.myPlayer == Projectile.owner && !CanSee2) Projectile.NewProjectile(Projectile.GetSource_FromThis(), player.Center.X, player.Center.Y + 20, 0, 0, LaunchHitBoxType, (int)(Projectile.damage), 0f, Projectile.owner, player.whoAmI, base.Projectile.whoAmI);
                Projectile.Kill();
            }
