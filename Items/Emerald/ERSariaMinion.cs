...
namespace SariaMod.Items.Emerald
{
    ...
    public class ERSariaMinion : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (base.projectile.localAI[0] == 0f)
            {
                ...
                for (int j = 0; j < 1; j++) //set to 2
                {
                    Projectile.NewProjectile(base.projectile.Center + Utils.RandomVector2(Main.rand, -24f, 24f), Vector2.One.RotatedByRandom(6.2831854820251465) * 1f, ModContent.ProjectileType<ZtargetE4>(), base.projectile.damage, base.projectile.knockBack, player.whoAmI, base.projectile.whoAmI);
                }
            ...
            if (player.HasBuff(ModContent.BuffType<EmeraldSariaBuff>())&&projectile.timeLeft<1798)
            {
            ...
            if ((player.HasBuff(ModContent.BuffType<EmeraldSariaBuff>()) && (player.ownedProjectileCounts[ModContent.ProjectileType<ETSariaMinion>()] > 0f)))
            {
                projectile.Kill();
            }
            ...
            if (projectile.frameCounter >= frameSpeed)
            {
                ...
                if (projectile.ai[0] == 2)
                {
                    ...
                    if (base.projectile.frame == 90)
                    {
                        ...
                        target = base.projectile.Center.MinionHoming(500f, player);// the distance she targets enemies
                        Main.PlaySound(SoundID.Item77, base.projectile.Center);
                        if (target != null) Projectile.NewProjectile(base.projectile.Center, base.projectile.DirectionTo(target.Center) * 10f, ModContent.ProjectileType<RubyPsychicSeeker>(), base.projectile.damage, 0f, base.projectile.owner);
                    }
