...
namespace SariaMod.Items.Ruby
{
    public class Explosion3 : ModProjectile
    {
        ...
        private const int CoreDiskSpawnInterval = 6;
        private const int CoreDiskSampleCount = 5;
        private const int InnerRingSpawnInterval = 4;
        private const int RedRingSpawnInterval = 3;
        private const int SmokeRingSpawnInterval = 5;
        private const int AccentDustSpawnInterval = 4;
        ...
        private readonly EruptionSmokeVisuals eruptionSmokeVisuals = new EruptionSmokeVisuals(24);
        private bool attackFinished;
        private int visualTailTimeLeft;
        private int smokePatternTick;

        public override string Texture => "SariaMod/Items/Ruby/Explosion2";
        ...
        public override void AI()
        {
            ...
            if (smokePatternTick % CoreDiskSpawnInterval == 0)
            {
                ...
                eruptionSmokeVisuals.Spawn(
                    ...
                    EruptionSmokeVisuals.CreateDustLikeVelocity() * 0.6f,
                    EruptionSmokeKind.Yellow);
            }

            if (Projectile.frame == 1 && smokePatternTick % InnerRingSpawnInterval == 0)
            {
            ...
            if (smokePatternTick % RedRingSpawnInterval == 0)
            {
                eruptionSmokeVisuals.SpawnEvenRing(
                    centerthis,
                    3.5f,
            ...
            if (smokePatternTick % SmokeRingSpawnInterval == 0)
            {
                eruptionSmokeVisuals.SpawnEvenRing(
                    centerthis,
                    5f,
            ...
            if (smokePatternTick % AccentDustSpawnInterval == 0)
            {
                Vector2 speed = Main.rand.NextVector2CircularEdge(1f, 1f);
                Dust d = Dust.NewDustPerfect(centerthis, ModContent.DustType<SmokeDust6>(), speed * 6f, Scale: 2.25f);
                d.noGravity = true;
            }
