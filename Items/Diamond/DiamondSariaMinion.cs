        ...
        public override void AI()
        {
            ...
            if (projectile.frame == 62 && (!player.HasBuff(ModContent.BuffType<StatLower>())) && (!player.HasBuff(ModContent.BuffType<Sickness>())))
            {
                if (Main.rand.NextBool())//controls the speed of when the sparkles spawn
                {
                    ...
                    Dust.NewDust(new Vector2((projectile.Center.X + sneezespot) + radius * (float)Math.Cos(angle), (projectile.Center.Y - 16) + radius * (float)Math.Sin(angle)), 0, 0, ModContent.DustType<Sneeze>(), 0f, 0f, 0, default(Color), 1.5f);
                }
            }
            if (projectile.frame == 62 && ((player.HasBuff(ModContent.BuffType<StatLower>())) || (player.HasBuff(ModContent.BuffType<Sickness>()))))
            {
            ...
            if ((Main.player[Main.myPlayer].active && Main.bloodMoon) && ((!player.HasBuff(ModContent.BuffType<Soothing>()))))
            {
                ...
                if (Main.rand.NextBool(20))//controls the speed of when the sparkles spawn
                {
                    ...
                    Dust.NewDust(new Vector2((projectile.Center.X + sneezespot) + radius * (float)Math.Cos(angle), (projectile.Center.Y - 16) + radius * (float)Math.Sin(angle)), 0, 0, ModContent.DustType<BlackSmoke>(), 0f, 0f, 0, default(Color), 1.5f);
                }
            }
            ...
            if ((Main.player[Main.myPlayer].active && Main.player[Main.myPlayer].ZoneSkyHeight || Main.player[Main.myPlayer].ZoneCorrupt || Main.player[Main.myPlayer].ZoneCrimson || !Main.dayTime))
            {
                if (!player.HasBuff(ModContent.BuffType<StatRaise>()) && !player.HasBuff(ModContent.BuffType<StatLower>()))
                {
            ...
            if (projectile.frameCounter >= frameSpeed)
            {
                ...
                if (projectile.ai[0] == 2)
                {
                    ...
                    {
                        if ((player.ownedProjectileCounts[ModContent.ProjectileType<MoonBlast>()] >= 1f))
                        {
                            ...
                            for (int j = 0; j < 1; j++) //set to 2
                            {
                                Projectile.NewProjectile(base.projectile.Center + Utils.RandomVector2(Main.rand, -24f, 24f), Vector2.One.RotatedByRandom(6.2831854820251465) * 4f, ModContent.ProjectileType<DazzlingGleam>(), base.projectile.damage, base.projectile.knockBack, player.whoAmI, base.projectile.whoAmI);
                            }
                        }
                        else if ((player.ownedProjectileCounts[ModContent.ProjectileType<MoonBlast>()] <= 0f))
                        {
                            Main.PlaySound(SoundID.Item77, base.projectile.Center);
                            {
                                {
