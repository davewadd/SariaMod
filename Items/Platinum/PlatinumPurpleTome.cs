...
namespace SariaMod.Items.Platinum
{
	...
	public class PlatinumPurpleTome : ModItem
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Platinum Tome (Purple Variant)");
		...
		public override void SetDefaults()
		{
			...
			item.noMelee = true;
			item.summon = true;
			item.buffType = ModContent.BuffType<PlatinumPurpleSariaBuff>();
			// No buffTime because otherwise the item tooltip would say something like "1 minute duration"
			item.shoot = ModContent.ProjectileType<PlatinumPurpleSariaMinion>();
		...
		{
			if (player.altFunctionUse == 2 && (player.ownedProjectileCounts[ModContent.ProjectileType<PlatinumPurpleSariaMinion>()] > 0f) && (player.ownedProjectileCounts[ModContent.ProjectileType<BarrierMinion>()] <= 1))
			{
			...
			else if (player.altFunctionUse != 2 && (player.ownedProjectileCounts[ModContent.ProjectileType<PlatinumPurpleSariaMinion>()] > 0f) && (player.ownedProjectileCounts[ModContent.ProjectileType<Psybeam>()] < 20f) && (player.ownedProjectileCounts[ModContent.ProjectileType<Psybeam>()] >= 17f))
			{
			...
			else if (player.altFunctionUse != 2 && (player.ownedProjectileCounts[ModContent.ProjectileType<PlatinumPurpleSariaMinion>()] > 0f) && (player.ownedProjectileCounts[ModContent.ProjectileType<Psybeam>()] < 17f))
			{
			...
			else if (player.altFunctionUse != 2 && (player.ownedProjectileCounts[ModContent.ProjectileType<PlatinumPurpleSariaMinion>()] <= 0f))
			{
		...
		{ 
			...
			if ((player.ownedProjectileCounts[ModContent.ProjectileType<PlatinumPurpleSariaMinion>()] > 0f) && (player.ownedProjectileCounts[ModContent.ProjectileType<BarrierMinion>()] <= 0))
			{
				Projectile.NewProjectile(position = Main.MouseWorld, new Vector2(speedX, speedY), ModContent.ProjectileType<BarrierMinion>(), damage / 4, knockBack, player.whoAmI);
			}
