...
{
	public class ZZBeam : ModProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / ZZBeamHold.NumBeams;
		...
		private const float MaxBeamSpread = 0f;
		...
		private const float BeamHitboxCollisionWidth = 10f;
		...
		private const float OuterBeamOpacityMultiplier = .5f;
		private const float InnerBeamOpacityMultiplier = .7f;
		...
		private const float BeamLightBrightness = 1.9f;
		...
		private const float BeamColorHue = 0.89f;
		...
		public override void AI() {
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / ZZBeamHold.MaxCharge, 0f, 1f);
			...
			Projectile.friendly = hostPrism.ai[0] > ZZBeamHold.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - ZZBeamHold.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			if (chargeRatio < 1f) {
				...
				if (chargeRatio <= 0.66f) {
					...
					spinRate = MathHelper.Lerp(2f, 1f, phaseRatio);
				}
				...
				// Spin rate increases dramatically during this time.
					...
					spinRate = MathHelper.Lerp(3f, 1f, phaseRatio);
				}
			}
			...
			// If the host Prism is already at max charge, don't calculate anything. Just use the max values.
				...
				beamSpread = 0f;
				spinRate = 1f;
				beamStartSidewaysOffset = 1f;
				beamStartForwardsOffset = 1f;
			}
			...
			Projectile.position += hostPrismDir;
		...
		private float PerformBeamHitscan(Projectile prism, bool fullCharge) {
			...
			Collision.LaserScan(samplingPoint, Projectile.velocity, 1 * Projectile.scale, MaxBeamLength, laserScanResults);
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			float visualBeamLength = BeamLength - 9f * Projectile.scale * Projectile.scale;
		...
		private Color GetOuterBeamColor() {
			...
			float hue = (BeamID / ZZBeamHold.NumBeams) % BeamHueVariance + BeamColorHue;
		...
		private Color GetInnerBeamColor() => Color.LightPink;
		...
		private void ProduceBeamDust(Color beamColor) {
			...
			double angle2 = Main.rand.NextDouble() * 5.0 * Math.PI;
			for (int i = 0; i < 10; i++)
			{
				Vector2 speed = Main.rand.NextVector2CircularEdge(1f, 1f);
				Dust d = Dust.NewDustPerfect(endPosition, ModContent.DustType<AbsorbPsychic>(), speed * -6, Scale: 2.5f);
				d.noGravity = true;
			}
